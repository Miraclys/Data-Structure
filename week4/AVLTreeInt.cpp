    ...
    if (compare(cur->value, val) == 0) {
        if (cur->frequency > 1) {
            ...
            if (cur->leftChild && cur->rightChild) {
                ...
                update(cur);
                if (cur->leftChild->getHeight() - cur->rightChild->getHeight() == 2) {
                    if (cur->leftChild->leftChild->getHeight() >= cur->rightChild->rightChild->getHeight()) {
                        leftPlus(cur);
                    } else {
                        leftRight(cur);
                    }
...
int main() {
    ...
    while (n--) {
        ...
        switch(opt) {
            ...
            case 3: std::cout << t.getRank(x) << "\n"; break;
            case 4: std::cout << t.getVal(y).first << "\n"; break;
            case 5: std::cout << t.getPre(x).first << "\n"; break;
            case 6: std::cout << t.getSuf(x).first << "\n"; break;
        }
    }
    return 0;
}
