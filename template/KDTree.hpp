...
KDTreeNode* KDTree::find_nearest_point(KDTreeNode* cur, Point target, int depth) {
    ...
    } else if (distance(ans->point, target) > distance(cur->point, target)) {
        ans = cur;
        if (anotherBranch) {
            KDTreeNode* opposite = find_nearest_point(anotherBranch, target, depth + 1);
            if (distance(target, ans->point) > distance(target, opposite->point))
    ...
    } else if (distance(target, ans->point) > std::abs(cur->point.coordinates[currentDim] - target.coordinates[currentDim])) {
        if (anotherBranch) {
            KDTreeNode* opposite = find_nearest_point(anotherBranch, target, depth + 1);
            if (distance(target, ans->point) > distance(target, opposite->point))
                ans = opposite;
